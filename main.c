...
float d_kp = 4, d_ki = 0.025, d_kd = 0;
float q_kp = 4, q_ki = 0.025, q_kd = 0;
...
//    U_out_dq.q=U_out_dq.q>dq_up?dq_up:(U_out_dq.q<dq_low?dq_low:U_out_dq.q);
    dq2alpha_beta(&U_out_dq,&theta, &U_out_alpha_beta);
    u_out_final=_IQ20div(U_out_alpha_beta.alpha,cons_rest_div);//逆变换/500归一
    u_out_final=fix_mpy(u_out_final,cons_ad_adj);//附加调制度
    u_out_final=u_out_final>cons_1?cons_1:(u_out_final<-cons_1?-cons_1:u_out_final);
    u_out_final=_IQ20mpy(cons_512,u_out_final);//转换成fpga里面的电压值
//    u_out_final=fix_mpy(u_out_final,cons_512);
